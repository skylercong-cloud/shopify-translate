...
- 运维概览不会返回或渲染 API key 明文，也不会返回数据库中的加密密文；当前阶段不提供网页端写配置能力。
...
当前翻译后台已支持 DeepSeek 主用、Qwen 备用、专业术语保护、预算限额、人工修正、后台重译和本地自动化验证。独立中文阅读界面、中英文切换、官方来源链接、未缓存页面按需采集、缓存文档的中英文统一搜索，以及带降级告警的只读运维概览均已接入。
