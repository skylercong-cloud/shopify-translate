...
- 搜索字段使用 PostgreSQL `pg_trgm` GIN 索引，并先取有界候选行再按 identifier/title/path/translation/source 评分，避免缓存语料增长后一次拉取过多命中行。
...
- 页面会显示最近的术语库版本历史，包括 active 标记、术语数量和创建时间，方便追踪术语调整。
- 页面可修改单用户登录密码；必须输入当前密码，成功后会撤销所有现有会话并回到登录页。
- 页面会显示当前有效会话数量，并可撤销其他设备或浏览器上的会话，同时保留当前会话。
- 页面会显示数据库写入健康告警；写入不可用时，翻译 Worker 会在调用模型前停止本轮翻译并稍后重试，避免浪费 API Token。
...
当前翻译后台已支持 DeepSeek 主用、Qwen 备用、专业术语保护、预算限额、人工修正、后台重译和本地自动化验证。独立中文阅读界面、中英文切换、官方来源链接、未缓存页面按需采集、带 PostgreSQL trigram 索引的缓存文档中英文统一搜索、网页端人工修正、翻译审核工作台、阅读页译文历史、带降级告警的运维概览、网页端 provider 设置、网页端 Prompt 激活、网页端运行参数设置、网页端术语库快照激活、网页端术语库历史查看、网页端术语库版本差异浏览、网页端密码修改、会话撤销、本地预览数据灌入、本地数据库备份与恢复验证命令，以及数据库写入不可用时的翻译安全阀均已接入。
